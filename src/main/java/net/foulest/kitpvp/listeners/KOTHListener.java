...
public class KOTHListener implements Listener {
    ...
    public static void onPlayerTeleport(PlayerTeleportEvent event) {
        ...
        if (KOTH.kothRefresh == null) {
            startTask(koth, player, event.getTo(), event.getFrom());
        }
    }
    ...
    public static void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (KOTH.kothRefresh == null) {
            startTask(KOTH.getActiveKoth(), player, event.getTo(), event.getFrom());
        }
...
//                MessageUtil.broadcastMessage("&9[KOTH] &6" + activeKOTH.getName() + " &eKOTH is being captured by &6"
//                        + activeKOTH.getCapper().getDisplayName() + " &c(" + KOTH.getTimeLeftNeat(activeKOTH) + ")");
//            }
//        }
    }

    public static void startTask(KOTH activeKOTH, Player player, Location to, Location from) {
        if (KOTH.kothRefresh == null) {
                ...
                public void run() {
                    if (KOTH.playersInKOTH.get(activeKOTH).size() > 1) {
                        if (!activeKOTH.isContested()) {
                            ...
                            MessageUtil.broadcastMessage("&9[KOTH] &6" + activeKOTH.getName() + " &eKOTH is being contested.");
                        }
                        ...
                        if (activeKOTH.getCapper() != null) {
                            ...
                            if (activeKOTH.getCapTime() - activeKOTH.getTimeLeft() >= 30) {
                                MessageUtil.broadcastMessage("&9[KOTH] &6" + activeKOTH.getName() + " &eKOTH is no longer being captured.");
                            }
                        }
